...
const tempCheck = (num) => {
    ...
    let boil = 212
    // set a variable to the boil value for simplicity
...
const onlyVowel = (string) => {
    ...
    for(i = 0; i < spStr.length; i++){
        ...
        if(spStr[i] === "a" || spStr[i] === "e" || spStr[i] === "i" || spStr[i] === "o" || spStr[i] === "u"){ 
            // checks to see if the element in the split up string is a vowel
...
console.log(noVowels(true))
console.log(noVowels(42))
console.log(noVowels("IAmACodingMaster"))
...
const alphaString = (array) => { 

    let maxLength = 0;
    // setting a variable which will store the max length of a given element within an array
    let lenArr = [];
    // initializing an empty array for later use 
    let ind;
    // initializing a variable that will set the index of the largest string
    for(i = 0; i < array.length; i++){

        lenArr.push(array[i].length);
        // adds the length of the current element to the empty array
        for(j = 0; j < lenArr.length; j++){
            // another for loop that will determine the highest number in the array
            if(lenArr[j] > maxLength){
                maxLength = lenArr[j]
                // set maxLength to the element that has been determined larger than the previous value of maxLength (update the value)
                ind = lenArr.indexOf(maxLength)
                // set the variable to the index of the largest number 
            }
        }
    }
    return array[ind]
...
// // trying a different train of thought that also works 
...
const cutOff = (array) => { // this function returns the inside portion 
...
// regular function (just wanted to practice)
const chopOff = (array) => {
